...
def normalize_enum_mappings(config: dict[str, Any]) -> list[dict[str, str]]:
    ...
    Expected per mapping:
      - column: source column with enum integer/code
      - optionset: Dynamics global option set name

    Optional:
      - metadata_table (default: globaloptionsetmetadata)
      - option_name_column (default: optionsetname)
      - option_value_column (default: optionvalue)
      - option_label_column (default: label)
      - output_column (default: <column>_label)
    """
...
def apply_enum_mappings(
    ...
    """Join Dynamics GlobalOptionSet metadata to produce label columns."""
